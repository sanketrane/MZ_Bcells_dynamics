...
//Function explainig change in influx rate over time
real phi_td(real time, real phi, real r_d )  
{ 
    real influx_td = phi * exp(-r_d * (time - 40));
    return influx_td;
  }
  ...
  vector Ode_sys(real time, //time
    ...
    real delta = ((phi_td(time, phi, r_d) * P)/M_stable) + rho; //loss rate calculated from the influx and division rate

    // the system of ODEs
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi_td(time, phi, r_d) * eps_donor(time) * chi_source(time - ageATBMT) * P
              + rho * (X[1] + 2 * X[2]) - ((1 / beta) + delta) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi_td(time, phi, r_d) * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * P
              + (1 / beta) * X[1] - (rho + delta) * X[2];
    //host Ki67+ cells
    dXdt[3] = phi_td(time, phi, r_d) * eps_host(time) * (1 - chi_source(time - ageATBMT)) * P
              + rho * (X[3] + 2 * X[4]) - ((1 / beta) + delta) * X[3];
    //host Ki67- cells
    dXdt[4] = phi_td(time, phi, r_d) * (1 - eps_host(time)) * (1 - chi_source(time - ageATBMT))* P 
             + (1 / beta) * X[3] - (rho+ delta) * X[4];
    return dXdt;
  }
...
parameters {
  ...
  real<lower=-0.05, upper=0.05> r_d;
...
model {
 // prior distributions for the parameters
  ...
  y3 ~ normal(logit_boundary_array(Kid_mean), sigma[3]);
  y4 ~ normal(logit_boundary_array(Kih_mean), sigma[4]);
}
...
generated quantities {
  ...
  for (i in 1 : numpred1) {
    ...
    donor_ki_pred1_err[i] = expit_boundary(normal_rng(logit_boundary(donor_ki_pred1[i]), sigma[3]));

    host_ki_pred1[i] = k_hat_pred1[4 * i - 1] / (k_hat_pred1[4 * i - 1] + k_hat_pred1[4 * i]);
    host_ki_pred1_err[i] = expit_boundary(normal_rng(logit_boundary(host_ki_pred1[i]), sigma[4]));
  }
  ...
  for (i in 1 : numpred2) {
    ...
    donor_ki_pred2_err[i] = expit_boundary(normal_rng(logit_boundary(donor_ki_pred2[i]), sigma[3]));
    
    host_ki_pred2[i] = k_hat_pred2[4 * i - 1] / (k_hat_pred2[4 * i - 1] + k_hat_pred2[4 * i]);
    host_ki_pred2_err[i] = expit_boundary(normal_rng(logit_boundary(host_ki_pred2[i]), sigma[4]));
  }

//Influx rate 
    array[numpred1] real influx_rate;
    for (i in 1:numpred1) {
          influx_rate[i] = phi_td(time_pred1_solve[i], phi, r_d );
   }
  ...
  y3_rep = normal_rng(logit_boundary_array(Kid_mean), sigma[3]);
  y4_rep = normal_rng(logit_boundary_array(Kih_mean), sigma[4]);
  ...
  for (i in 1 : numobs) {
    ...
    log_lik_Kid[i] = normal_lpdf(y3[i] | logit_boundary(Kid_mean[i]), sigma[3]);
    log_lik_Kih[i] = normal_lpdf(y4[i] | logit_boundary(Kih_mean[i]), sigma[4]);
...
// Calculate residuals for the observed data
  ...
  for (i in 1 : numobs) {
    ...
    residuals_Kid[i] = y3[i] - logit_boundary(Kid_mean[i]);
    residuals_Kih[i] = y4[i] - logit_boundary(Kih_mean[i]);
  }
}