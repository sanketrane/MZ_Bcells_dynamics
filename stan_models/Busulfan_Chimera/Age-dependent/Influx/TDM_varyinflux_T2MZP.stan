...
{ 
  ...
  real beta = 3.5; //Loss of Ki67+ cells
    ...
    //host Ki67+ cells
    dXdt[3] = phi_td(time, phi, r_d) * eps_host * (1 - chi_source(time - ageATBMT)) * P
              + rho * (X[3] + 2 * X[4]) - ((1 / beta) + delta) * X[3];
    //host Ki67- cells
    dXdt[4] = phi_td(time, phi, r_d) * (1 - eps_host) * (1 - chi_source(time - ageATBMT))* P 
