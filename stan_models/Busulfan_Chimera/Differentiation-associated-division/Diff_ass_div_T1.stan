functions {
  ...
  vector Ode_sys(real time, //time
    ...
    real delta = ((phi* P)/M_stable) + rho; //influx rate calculated from the loss and division rate
    
    // the system of ODEs
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi * eps_donor * chi_source(time - ageATBMT) * P
              + rho * (X[1] + 2 * X[2]) - ((1 / beta) + delta) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi * (1 - eps_donor) * chi_source(time - ageATBMT) * P
