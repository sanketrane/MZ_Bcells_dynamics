...
real chi_source(real time) {
  ...
  real eps_donor = 1; //mean of the donor Ki67+ cells
  real eps_host = 1; //mean of the host Ki67+ cells
  real beta = 3.5; //Loss of Ki67+ cells
   ...
   real delta = ((phi* theta_spline)/M_stable) + rho; //influx rate calculated from the loss and division rate
    ...
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi * eps_donor * chi_source(time - ageATBMT) * theta_spline
    + rho * (X[1] + 2 * X[2]) - ((1 / beta) + delta) * X[1];
     //donor Ki67- cells
     dXdt[2] = phi * (1 - eps_donor) * chi_source(time - ageATBMT) * theta_spline
