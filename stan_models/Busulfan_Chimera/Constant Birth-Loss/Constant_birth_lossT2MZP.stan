functions {
// Function that describes changes in the precursor population
real chi_source(real time) {
   ...
   if ((time - t0) < 0){
       ...
       chi_s = chi_est * (1 - exp(-mu * (time - t0)^2));
     }
    return chi_s;
  }

  //Function that describes changes Ki67+ chimerism in the donor source
  real eps_donor(real time) {
    ...
    real eps_d;
      ...
      eps_d = (a0* exp(-b0 * (time - t0))) + epid_0;
  ...
  vector Ode_sys(real time, //time
    ...
    real ageATBMT = parms[3]; //age at BMT
    
  // fixed parameters
  real theta_spline = 1788129;
  real eps_host = 0.078; //mean of the host Ki67+ cells
  real beta = 3.5; //Loss of Ki67+ cells

   real M_stable = X[1] + X[2] + X[3] + X[4]; // total counts at steady state
   real delta = ((phi* theta_spline)/M_stable) + rho; //influx rate calculated from the loss and division rate
    
  
    // the system of ODEs
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi * eps_donor(time) * chi_source(time - ageATBMT) * theta_spline
    + rho * (X[1] + 2 * X[2]) - ((1 / beta) + delta) * X[1];
     //donor Ki67- cells
     dXdt[2] = phi * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * theta_spline
     + (1 / beta) * X[1] - (rho + delta) * X[2];
     //host Ki67+ cells
      dXdt[3] = phi * eps_host * (1 - chi_source(time - ageATBMT)) * theta_spline
      + rho * (X[3] + 2 * X[4]) - ((1 / beta) + delta) * X[3];
     //host Ki67- cells
     dXdt[4] = phi * (1 - eps_host) * (1 - chi_source(time - ageATBMT)) * theta_spline
  ...
  vector initial_cond_generator(vector initial_cond, real solve_time,
    ...
    array[3] real par_tb; // parameters for the intial_cond_generator
    par_tb[1 : 2] = parms[1 : 2];
    par_tb[3] = 40.0; // use the earliest age at BMT 
  ...
  vector prediction_generator(vector initial_cond, real solve_time,
    ...
    solution = ode_rk45(Ode_sys, y0, ageatBMT, rep_array(solve_time, 1),
                        par_tb);
    return to_vector(solution[1]);
  }
  ...
  vector map_rect_helper_func(vector eta, vector theta, array[] real x_r,
    ...
    array[2] real parms = to_array_1d(eta[5 : 6]);
  ...
  array[] vector ode_solver_parallel(vector initial_cond,
    ...
    vector[6] eta = append_row(initial_cond, to_vector(parms));
...
parameters {
  ...
  real<lower=0> rho; //loss rate
  real<lower=0> phi; //division rate
}
...
transformed parameters {
  ...
  array[2] real parms; // declaring the array for parameters
...
generated quantities {
  ...
  array[numpred1] real countspred_age1;
  ...
  real rho_inv = 1 / rho; // inter-division time
