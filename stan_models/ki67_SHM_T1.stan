functions{
  ...
  real theta_spline(real time){
      real nu = 9.32E-4; real theta0 = 14.20;
      real theta;
   ...
   real Chi_T1( real time){
    ...
    real[] shm(real time,  real[] k, real[] parms, real[] rdata, int[] idata) {
     ...
     //donor ki67hi
     dkdt[1] = (psi * theta_spline(time) * Chi_T1(time-ageAtBMT) * eps_donor) + rho * (2 * k[2] + k[1]) - ((1/Beta) + (lambda + rho)) * k[1];
     //donor ki67lo
     dkdt[2] = (psi * theta_spline(time) * Chi_T1(time-ageAtBMT) * (1-eps_donor)) + (1/Beta) * k[1] - (rho + (lambda + rho)) * k[2];
     //host ki67hi
     dkdt[3] = (psi * theta_spline(time) * (1-Chi_T1(time-ageAtBMT)) * eps_host) + rho * (2 * k[4] + k[3]) - ((1/Beta) + (lambda + rho)) * k[3];
     //host ki67lo
     dkdt[4] = (psi * theta_spline(time) * (1-Chi_T1(time-ageAtBMT)) * (1-eps_host)) + (1/Beta) * k[3] - (rho + (lambda + rho)) * k[4];
