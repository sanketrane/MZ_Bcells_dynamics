functions{
  ...
  real theta_spline(real time){
      ...
      real nu = 4.4E-4; real theta0 = 14.41;
   ...
   real Chi_T2_timevar( real time){

      real chiEst = 0.8; real qEst = 0.1383;
      int t0 = 10;
   ...
   real Chi_T1( real time){
      ...
      real chiEst = 0.8; real qEst = 0.1748;
      int t0 = 10;
    ...
    real[] shm(real time,  real[] k, real[] parms, real[] rdata, int[] idata) {
     ...
     real eps_donor = 0.94; real eps_host = 0.79;
     ...
     //donor ki67hi
     dkdt[1] = (psi * theta_spline(time) * Chi_T2_timevar(time-ageAtBMT) * eps_donor) + rho * (2 * k[2] + k[1]) - ((1/Beta) + (lambda + rho)) * k[1];
     //donor ki67lo
     dkdt[2] = (psi * theta_spline(time) * Chi_T2_timevar(time-ageAtBMT) * (1-eps_donor)) + (1/Beta) * k[1] - (rho + (lambda + rho)) * k[2];
     //host ki67hi
     dkdt[3] = (psi * theta_spline(time) * (1-Chi_T2_timevar(time-ageAtBMT)) * eps_host) + rho * (2 * k[4] + k[3]) - ((1/Beta) + (lambda + rho)) * k[3];
     //host ki67lo
     dkdt[4] = (psi * theta_spline(time) * (1-Chi_T2_timevar(time-ageAtBMT)) * (1-eps_host)) + (1/Beta) * k[3] - (rho + (lambda + rho)) * k[4];
