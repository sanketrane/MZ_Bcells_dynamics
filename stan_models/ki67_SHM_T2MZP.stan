functions{
  ...
  real theta_spline(real time){
   ...
   real Chi_T2MZP_timevar( real time){
   ...
   real Chi_T1( real time){
    ...
    real eps_donor_timevar( real time){
       ...
       real eps_0 = 0.068; real eps_f  = 0.01; real A = 115;
    ...
    real[] shm(real time,  real[] k, real[] parms, real[] rdata, int[] idata) {
     ...
     real dkdt[4];
     //donor ki67hi
     dkdt[1] = (psi * theta_spline(time) * Chi_T2MZP_timevar(time-ageAtBMT) * eps_donor_timevar(time)) + rho * (2 * k[2] + k[1]) - ((1/Beta) + (lambda + rho)) * k[1];
     //donor ki67lo
     dkdt[2] = (psi * theta_spline(time) * Chi_T2MZP_timevar(time-ageAtBMT) * (1-eps_donor_timevar(time))) + (1/Beta) * k[1] - (rho + (lambda + rho)) * k[2];
     //host ki67hi
     dkdt[3] = (psi * theta_spline(time) * (1-Chi_T2MZP_timevar(time-ageAtBMT)) * eps_host) + rho * (2 * k[4] + k[3]) - ((1/Beta) + (lambda + rho)) * k[3];
     //host ki67lo
     dkdt[4] = (psi * theta_spline(time) * (1-Chi_T2MZP_timevar(time-ageAtBMT)) * (1-eps_host)) + (1/Beta) * k[3] - (rho + (lambda + rho)) * k[4];
