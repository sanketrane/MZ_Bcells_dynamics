functions{
  ...
  real[] tdd(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
   ...
   // the array of input data (fixed varibaled) invloved in ode equations
    ...
    // for age correction -- 'tb' is the age of host at BMT -- 'ta' is the age at BMT of the yougest hosts.
     int ta = 40;
     real tb = idata[1];
     ...
     dkdt[1] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * eps_donor) + (rho * exp(-r_d * (t-ta)) * (2 * k[2] + k[1])) - ((1/Beta) + delta) * k[1];
     dkdt[2] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * (1-eps_donor)) + (1/Beta) * k[1] - (rho * exp(-r_d * (t-ta)) + delta) * k[2];

     dkdt[3] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * eps_host) + (rho * exp(-r_d * (t-ta)) * (2 * k[4] + k[3])) - ((1/Beta) + delta) * k[3];
     dkdt[4] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * (1-eps_host)) + (1/Beta) * k[3] - (rho * exp(-r_d * (t-ta)) + delta) * k[4];
...
model{
  ...
  psi ~ normal(0.1, 0.5);
  y0_Log ~ normal(14, 1);
  kappa_0 ~  normal(0.1, 0.2);
  rho_Log ~ normal(-4, 1.2);
  delta_Log ~ normal(-4, 1.2);
  r_d ~ normal(0.01, 0.01);
  Beta ~ normal(3.5, 0.8);
