functions{
  ...
  real theta_spline(real t,
   ...
   real Chi_spline( real t,
     ...
     real[] svm(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
   ...
   real[] foreach_init(real tb, real ta, real[] init_cond, real[] parms, real[] rdata, int x_i){
     ...
     return to_array_1d(integrate_ode_rk45(svm, init_cond, ta, rep_array(tb, 1), parms, rdata, rep_array(x_i, 1)));
   }
   ...
   real[] foreach_ode(real ts, real t0, real[] init_cond, real[] parms, real[] rdata, int x_i) {
     //first argument to integrate_ode_rk45 must be the name of the ODE function with signature (real, real[], real[], real[], int[])
     return to_array_1d(integrate_ode_rk45(svm, init_cond, t0, rep_array(ts, 1), parms, rdata, rep_array(x_i, 1)));
    }
...
parameters{
  ...
  transformed parameters{
    ...
    real r_psi = exp(r_psi_Log);      // transformed parameters for better/faster sampling
  ...
  model{
    ...
    rho_Log ~ normal(-4, 1);
    delta_Log ~ normal(-3, 1);
    Beta ~ normal(3.5, 0.8);
    r_psi_Log ~ normal(-5, 1);
