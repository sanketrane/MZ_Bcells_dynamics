functions {
 ...
 real phi_m(real M, real phi, real K)  
 {   
    return phi * (1 - (M/K));
  }
  ...
  vector Ode_sys_ont(real time, //time
    ...
    vector[1] dMdt;
    dMdt[1] = phi_m(M[1], phi, K) * P(time) + b * M[1] - d * M[1];
    return dMdt;
  }
 ...
 //Function that contain the ODE equations to be used in the ODE solver
  vector Ode_sys(real time, //time
    ...
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi_m(X[1] + X[2]+ X[3] + X[4], phi, K) * eps_donor(time) * chi_source(time - ageATBMT) * P(time)
              + b * (X[1] + 2 * X[2]) - ((1 / beta) + d) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi_m(X[1] + X[2]+ X[3] + X[4], phi, K) * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * P(time)
              + (1 / beta) * X[1] - (b + d) * X[2];
    //host Ki67+ cells
    dXdt[3] = phi_m(X[1] + X[2]+ X[3] + X[4], phi, K) * eps_host(time) * (1 - chi_source(time - ageATBMT)) * P(time)
              + b * (X[3] + 2 * X[4]) - ((1 / beta) + d) * X[3];
    //host Ki67- cells
    dXdt[4] = phi_m(X[1] + X[2]+ X[3] + X[4], phi, K) * (1 - eps_host(time)) * (1 - chi_source(time - ageATBMT))* P(time)
             + (1 / beta) * X[3] - (b + d) * X[4];
    return dXdt;
  }
...
transformed parameters {
  ...
  real K = exp(K_Log);
  array[4] real parms_ont = {b, d, K, phi};
  array[5] real parms = {b, d, K, phi,0.16}; // parameters to be passed to the ODE solver
