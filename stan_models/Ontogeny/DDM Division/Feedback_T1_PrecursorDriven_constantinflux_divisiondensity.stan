functions {
  ...
  real constrained_d(real b, real M_stable, real phi) {
    real S = exp(14.10);
    real d = (phi * S / M_stable) + b * M_stable;
    if (d <=  2 * sqrt(b * S))
      reject("d must satisfy d > 2*sqrt(b*S)");
    return d;
  }
  ...
  vector Ode_sys_ont(real time, //time
    ...
    real d = constrained_d(b, M_stable, phi); // death rate is set to balance the influx at the stable MZ count
  
  // ODE for calulating the change in MZ counts over time
    vector[1] dMdt;
    dMdt[1] = phi * P(time) + b_m(M[1], b) * M[1] - d * M[1];
    return dMdt;
  }
 ...
 //Function that contain the ODE equations to be used in the ODE solver
  vector Ode_sys(real time, //time
    ...
    // fixed parameters
    real beta = 3.5;
    real d = constrained_d(b, M_stable, phi);
    // the system of ODEs
    vector[4] dXdt;
    //donor Ki67+ cells
 ...
 //we define a solver to generate initial conditions at each age at BMT, using the earliest age at BMT as t_0
  ...
  N0[1] = ode_pred_ont(40, parms[1:3]); // initial conditions at t=10
...
transformed parameters {
  ...
  real M_stable = exp(M_ln); // transforming the parameter to ensure positivity
  array[3] real parms_ont = {b, M_stable, phi};
  array[4] real parms = {b, M_stable, phi, 0.16}; // parameters to be passed to the ODE solver
