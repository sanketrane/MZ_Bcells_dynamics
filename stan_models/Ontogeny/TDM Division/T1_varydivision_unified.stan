functions {
  ...
  real P(real time) {
   ...
   real b_t(real time, real b, real r_d) {
  ...
  vector Ode_sys_ont(real time, //time
    ...
    real r = parms[4]; // Population size at which the death rate had doubled
    ...
    vector[1] dMdt;
    dMdt[1] = phi * P(time) + b_t(time, b, r) * M[1] - d * M[1];
    return dMdt;
  }
 ...
 //Function that contain the ODE equations to be used in the ODE solver
  vector Ode_sys(real time, //time
    ...
    real b = parms[2]; //division rate
    real d = parms[3]; // death rate
    real r_d = parms[4]; // Population size at which the death rate had doubled
    ...
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi * eps_donor(time) * chi_source(time - ageATBMT) * P(time)
              + b_t(time, b, r_d) * (X[1] + 2 * X[2]) - ((1 / beta) + d) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * P(time)
              + (1 / beta) * X[1] - (b_t(time, b, r_d) + d) * X[2];
    //host Ki67+ cells
    dXdt[3] = phi * eps_host(time) * (1 - chi_source(time - ageATBMT)) * P(time)
              + b_t(time, b, r_d) * (X[3] + 2 * X[4]) - ((1 / beta) + d) * X[3];
    //host Ki67- cells
    dXdt[4] = phi * (1 - eps_host(time)) * (1 - chi_source(time - ageATBMT))* P(time)
             + (1 / beta) * X[3] - (b_t(time, b, r_d) + d) * X[4];
    return dXdt;
  }
...
parameters {
  ...
  real<lower=0> d; 
  real<lower=0> b;
  real<lower=0> r_d; // Population size at which the division rate had halved
...
transformed parameters {
  ...
  array[4] real parms_ont = {phi, b, d, r_d};
  array[5] real parms = {phi, b, d, r_d,0.16}; // parameters to be passed to the ODE solver
