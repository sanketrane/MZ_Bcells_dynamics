functions {
 ...
 real phi_t(real time)  
 {   
    real phi_max = 0.0168; //Estimated influx rate from the chimera data
    real phi0 = 0.002; // influx rate at time t=11, which is the minimum time point in the dataset
    real r =31;
    return ((phi_max - phi0)/ (1 + (r/ time)^8)) + phi0; 
  }
  ...
  vector Ode_sys_ont(real time, //time
    ...
    vector[1] dMdt;
    dMdt[1] = phi_t(time) * P(time) + b_m(M[1], b, K) * M[1] - d * M[1];
    return dMdt;
  }
 ...
 //Function that contain the ODE equations to be used in the ODE solver
  vector Ode_sys(real time, //time
    ...
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi_t(time) * eps_donor(time) * chi_source(time - ageATBMT) * P
              + b_m((X[1] + X[2]+ X[3] + X[4]), b, K) * (X[1] + 2 * X[2]) - ((1 / beta) + d) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi_t(time) * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * P
              + (1 / beta) * X[1] - (b_m((X[1] + X[2]+ X[3] + X[4]), b, K) + d) * X[2];
    //host Ki67+ cells
    dXdt[3] = phi_t(time) * eps_host(time) * (1 - chi_source(time - ageATBMT)) * P
              + b_m((X[1] + X[2]+ X[3] + X[4]), b, K) * (X[3] + 2 * X[4]) - ((1 / beta) + d) * X[3];
    //host Ki67- cells
    dXdt[4] = phi_t(time) * (1 - eps_host(time)) * (1 - chi_source(time - ageATBMT))* P
 ...
 //we define a solver to generate initial conditions at each age at BMT, using the earliest age at BMT as t_0
  ...
  initial_cond[3] = N0[1] * parms[4]; // host Ki67+ cells
  initial_cond[4] = N0[1] * (1 - parms[4]); // host Ki67- cells
  ...
  vector map_rect_helper_func(vector eta, vector theta, array[] real x_r,
   array[] int x_i) {
  array[4] real parms = to_array_1d(eta[1 : 4]);
...
transformed parameters {
  ...
  real K = exp(K_Log);
  array[3] real parms_ont = {b, d, K};
  array[4] real parms = {b, d, K, kappa_0}; // parameters to be passed to the ODE solver
