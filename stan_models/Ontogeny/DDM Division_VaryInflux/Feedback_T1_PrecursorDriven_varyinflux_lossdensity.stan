functions {
  ...
  //Varying the loss rate with the MZ counts
   real d_m(real M, real d, real K) {
   real death_m = d * (1 + (M/K));
   return death_m;
    }

   //Function explainig division
 real phi_t(real time, real phi, real phi0, real r)  
 {   
    // real phi_max = 0.019; //Estimated influx rate from the chimera data
    // real phi0 = 0.002; // influx rate at time t=11, which is the minimum time point in the dataset
    // real r =34;
  ...
  vector Ode_sys_ont(real time, //time
    ...
    real b = parms[1]; // division rate
    real d = parms[2]; // death rate
    real K = parms[3]; // Population size at which the death rate had doubled
    real phi = parms[4]; // influx rate
    real phi0 = parms[5]; // influx rate at time t=11
    real r = parms[6]; // parameter for the phi_t function
  
    // ODE for calulating the change in MZ counts over time
    vector[1] dMdt;
    dMdt[1] = phi_t(time, phi, phi0, r) * P(time) - d_m(M[1], d, K) * M[1] +b * M[1];
    return dMdt;
  }
 ...
 //Function that contain the ODE equations to be used in the ODE solver
  vector Ode_sys(real time, //time
    ...
    real phi0 = parms[5]; // influx rate at time t=11
    real r = parms[6]; // parameter for the phi_t function
    real ageATBMT = parms[7]; //age at BMT
    ...
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi_t(time, phi, phi0, r) * eps_donor(time) * chi_source(time - ageATBMT) * P
              + b * (X[1] + 2 * X[2]) - ((1 / beta) + d_m((X[1] + X[2]+ X[3] + X[4]), d, K)) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi_t(time, phi, phi0, r) * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * P
              + (1 / beta) * X[1] - (b + d_m((X[1] + X[2]+ X[3] + X[4]), d, K)) * X[2];
    //host Ki67+ cells
    dXdt[3] = phi_t(time, phi, phi0, r) * eps_host(time) * (1 - chi_source(time - ageATBMT)) * P
              + b * (X[3] + 2 * X[4]) - ((1 / beta) + d_m((X[1] + X[2]+ X[3] + X[4]), d, K)) * X[3];
    //host Ki67- cells
    dXdt[4] = phi_t(time, phi, phi0, r) * (1 - eps_host(time)) * (1 - chi_source(time - ageATBMT))* P
             + (1 / beta) * X[3] - (b + d_m((X[1] + X[2]+ X[3] + X[4]), d, K)) * X[4];
    return dXdt;
  }
 ...
 //we define a solver to generate initial conditions at each age at BMT, using the earliest age at BMT as t_0
  vector initial_cond_generator(vector initial_cond, real solve_ageatBMT,
    ...
    array[7] real par_tb; // parameters for the intial_cond_generator
    par_tb[1 : 6] = parms[1 : 6];
    par_tb[7] = 40.0; // use the earliest age at BMT 
  ...
  N0[1] = ode_pred_ont(40, parms[1:6]); // initial conditions at t=10
  // initial conditions at time t=11
  initial_cond[1] = 0; //MZ counts at t=11
  initial_cond[2] = 0; // donor Ki67- cells
  initial_cond[3] = N0[1] * parms[7]; // host Ki67+ cells
  initial_cond[4] = N0[1] * (1 - parms[7]); // host Ki67- cells
  ...
  vector map_rect_helper_func(vector eta, vector theta, array[] real x_r,
   array[] int x_i) {
  array[7] real parms = to_array_1d(eta[1 : 7]);
...
parameters {
  ...
  real<lower=0> b;
  real<lower=0> phi; 
  real<lower=11, upper=40> r;
  real<lower=0, upper=phi> phi0;
...
transformed parameters {
  ...
  real K = exp(K_Log);
  array[6] real parms_ont = {b, d, K, phi, phi0, r};
  array[7] real parms = {b, d, K, phi, phi0, r, 0.16}; // parameters to be passed to the ODE solver
