functions {
  ...
  real P(real time) {
   ...
   real b_m(real M, real b) {
   real r_m = b * M;
   return r_m;
    }

   //Function explainig influx
 ...
 {   
  ...
  vector Ode_sys_ont(real time, //time
    ...
    real M_stable = parms[2]; // death rate
    real phi = parms[3]; // influx rate
    real phi0 = parms[4]; // influx rate at time t=11
    real r = parms[5]; // parameter for the phi_t function
  
    real S = 1298783; //mean of the precursor counts
    real d = b * M_stable + (phi*S/M_stable); // calculating the death rate based on the division rate and the stable population size
    // ODE for calulating the change in MZ counts over time
    vector[1] dMdt;
    dMdt[1] = phi_t(time, phi, phi0, r) * P(time) + b_m(M[1], b) * M[1] - d * M[1];
    return dMdt;
  }
 ...
 //Function that contain the ODE equations to be used in the ODE solver
  vector Ode_sys(real time, //time
    ...
    real phi0 = parms[4]; // influx rate at time t=11
    real r = parms[5]; // parameter for the phi_t function
    real ageATBMT = parms[6]; //age at BMT
    ...
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi_t(time, phi, phi0, r) * eps_donor(time) * chi_source(time - ageATBMT) * P(time)
              + b_m((X[1] + X[2]+ X[3] + X[4]), b) * (X[1] + 2 * X[2]) - ((1 / beta) + d) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi_t(time, phi, phi0, r) * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * P(time)
              + (1 / beta) * X[1] - (b_m((X[1] + X[2]+ X[3] + X[4]), b) + d) * X[2];
    //host Ki67+ cells
    dXdt[3] = phi_t(time, phi, phi0, r) * eps_host(time) * (1 - chi_source(time - ageATBMT)) * P(time)
              + b_m((X[1] + X[2]+ X[3] + X[4]), b) * (X[3] + 2 * X[4]) - ((1 / beta) + d) * X[3];
    //host Ki67- cells
    dXdt[4] = phi_t(time, phi, phi0, r) * (1 - eps_host(time)) * (1 - chi_source(time - ageATBMT))* P(time)
             + (1 / beta) * X[3] - (b_m((X[1] + X[2]+ X[3] + X[4]), b) + d) * X[4];
    return dXdt;
  }
 ...
 //we define a solver to generate initial conditions at each age at BMT, using the earliest age at BMT as t_0
  vector initial_cond_generator(vector initial_cond, real solve_ageatBMT,
    ...
    array[6] real par_tb; // parameters for the intial_cond_generator
    par_tb[1 : 5] = parms[1 : 5];
    par_tb[6] = 40.0; // use the earliest age at BMT 
  ...
  N0[1] = ode_pred_ont(40, parms[1:5]); // initial conditions at t=10
  // initial conditions at time t=11
  initial_cond[1] = 0; //MZ counts at t=11
  initial_cond[2] = 0; // donor Ki67- cells
  initial_cond[3] = N0[1] * parms[6]; // host Ki67+ cells
  initial_cond[4] = N0[1] * (1 - parms[6]); // host Ki67- cells
  ...
  vector map_rect_helper_func(vector eta, vector theta, array[] real x_r,
   array[] int x_i) {
  array[6] real parms = to_array_1d(eta[1 : 6]);
...
parameters {
  ...
  real<lower=0, upper=15.3> M_Log; // Population size at which the death rate had doubled
...
transformed parameters {
  ...
  real M_stable = exp(M_Log);
  array[5] real parms_ont = {b, M_stable, phi, phi0, r};
  array[6] real parms = {b, M_stable, phi, phi0, r, 0.16}; // parameters to be passed to the ODE solver
