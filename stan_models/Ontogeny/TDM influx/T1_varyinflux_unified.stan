functions {
 ...
 real phi_t(real time, real phi, real t0, real r_phi)  
 {  
    return (phi / (1 + exp(-r_phi * (time - t0)))); 
  }
  ...
  vector Ode_sys_ont(real time, //time
    ...
    real t0 = parms[4]; // time at which the influx rate is maximum
    real r_phi = parms[5]; // parameter for the phi_t function
  
  
    // ODE for calulating the change in MZ counts over time
    vector[1] dMdt;
    dMdt[1] = phi_t(time, phi, t0, r_phi) * P(time) + b * M[1] - d * M[1];
    return dMdt;
  }
 ...
 //Function that contain the ODE equations to be used in the ODE solver
  vector Ode_sys(real time, //time
    ...
    vector[4] dXdt;
    //donor Ki67+ cells
    dXdt[1] = phi_t(time, phi, t0, r_phi) * eps_donor(time) * chi_source(time - ageATBMT) * P(time)
              + b * (X[1] + 2 * X[2]) - ((1 / beta) + d) * X[1];
    //donor Ki67- cells
    dXdt[2] = phi_t(time, phi, t0, r_phi) * (1 - eps_donor(time)) * chi_source(time - ageATBMT) * P(time)
              + (1 / beta) * X[1] - (b + d) * X[2];
    //host Ki67+ cells
    dXdt[3] = phi_t(time, phi, t0, r_phi) * eps_host(time) * (1 - chi_source(time - ageATBMT)) * P(time)
              + b * (X[3] + 2 * X[4]) - ((1 / beta) + d) * X[3];
    //host Ki67- cells
    dXdt[4] = phi_t(time, phi, t0, r_phi) * (1 - eps_host(time)) * (1 - chi_source(time - ageATBMT))* P(time)
             + (1 / beta) * X[3] - (b + d) * X[4];
    return dXdt;
  }
...
transformed parameters {
  ...
  array[5] real parms_ont = {b, d, phi, t0, r_phi};
  array[6] real parms = {b, d, phi, t0, r_phi, 0.16}; // parameters to be passed to the ODE solver
