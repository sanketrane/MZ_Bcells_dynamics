functions {
  ...
  //Function that describes changes in the source T1 counts over time
  real P(real time) {
    real P_est = 14.10;
    real n = 2.0;
    real nu = 0.31;
    ...
    //Varying the loss rate over time
    real d_t(real time, real d, real r_d, real t0)
    {
      real lam_var;
      if (time < t0){
      lam_var = d * exp(-r_d * (time-t0));
    } else {
      lam_var = d;
    }
      return(lam_var);
  ...
  vector Ode_sys_ont(real time, //time
    ...
    real t0 = parms[5]; // time at which the loss rate starts to vary
   
    // ODE for calulating the change in MZ counts over time
    vector[1] dMdt;
    dMdt[1] = phi * P(time) + b * M[1] - d_t(time, d, r_d, t0) * M[1];
    return dMdt;
  }
...
parameters {
  ...
  real<lower=0> t0; // time at which the loss rate starts to vary
...
transformed parameters {
  ...
  array[5] real parms_ont = {phi, b, d, r_d, t0};
  array[6] real parms = {phi, b, d, r_d, t0, 0.16}; // parameters to be passed to the ODE solver
