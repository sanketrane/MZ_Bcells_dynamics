## Random loss model with time-varying source influx-- Homogeneous model 
...
## model files (Stan)
modelName <- "ki67_SVM_MZ"        # name of the file for stan model 
...
init <- function() list(
  ...
  rho_Log = rnorm(1, -4, 0.1),
  delta_Log = rnorm(1, -4, 0.1),
  r_psi_Log = rnorm(1, -4, 0.1),
...
parametersToPlot <- c("psi", "rho", "delta", "Beta", "r_psi", "kappa_0", "y0_Log",
