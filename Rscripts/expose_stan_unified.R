...
udfs <- expose_cmdstanr_functions("stan_models/new_models/Steady_State/2_AgeBins/Ontogeny+Chimeras/Linear.neutral.feedback.T1fitall.stan")
# udes <- expose_cmdstanr_functions("stan_models/Linear.neutral.TAA4.1.stan")
# print(udfs$chi_source(12))
# print(udfs$b_m(exp(13), 0.45, exp(14)))

initial_conds <- c(0, 0, 1187133, 6083180)
initial_cond1 <- c( 0, 0, 106.3357, 92984.5643)
parms_ont  <- c(0.009, 0.13, 0.025, exp(14.75))
parms <- c(0.009, 0.13, 0.025, exp(14.75), 9250, 0.16)
# parms2 <- c(0.01, 0.01, 0.01, 0.01, 67)
# parms3 <- c(0.01, 0.01, 0.01, 0.01, 88)
#ageatbmt <- c(45, 52, 55)

solve_time1 <- c(59, 62, 69, 76, 88, 92, 95, 102, 109, 119, 123, 124, 141, 158, 212, 219, 291, 306, 731)
solve_ageatBMT <- c(41, 41, 41, 41, 74, 74, 74, 74, 74, 41, 41, 42, 51, 65, 58, 65, 101, 89, 87)
...
initial_gen <- udfs$ode_pred_ont(40, 9250, parms_ont)
print(initial_gen)
...
mysol <- udfs$ode_pred(solve_time1, 41, parms)

# mysol_T2 <- udfs$ode_solver_parallel(solve_time, parms)
...
for(i in 1:length(solve_time1))
{ Mz_total <- X1[i]+ X2[i]+X3[i] + X4[i]
  Nfd <- (X1[i]+X2[i])/(Mz_total * udfs$chi_source(solve_time1[i]-41))
  print(paste("At time", solve_time1[i], "Total Mz:", Mz_total, "Nfd:", Nfd))
}
